...
count = 1
file_sizes = []
tokenizer, model, fill_mask, nlp_features = load_finetuned_llm()
...
for filename in os.listdir(directory):
    ...
    for f in funcslist:
        ...
        preprocess_for_stripped_binaries(count, "obf", filename, df, data, fill_mask)
        funs += 1
        if funs == func_limit:
            break
    count += 1
...
#             api = json.loads(line)
#             api_n = api["api_name"]
#             if 'dword' not in api_n and '?' not in api_n and '_' not in api_n and '@' not in api_n and 'eax' not in api_n and 'ecx' not in api_n and 'edi' not in api_n and 'edx' not in api_n and 'ebp' not in api_n and 'esp' not in api_n and 'ebx' not in api_n and 'esi' not in api_n:
#                 api_count += 1
#                 if 'ds:' in api_n:
#                     api_n = api_n.split(':')[1:]
#                     api_n = ' '.join(api_n)
